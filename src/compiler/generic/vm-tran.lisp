...
(deftransform count ((item sequence &key (start 0) end)
  ...
  (if (and (or (not start) (and (constant-lvar-p start) (eq (lvar-value start) 0)))
           (eq end nil)
           (eq (lvar-type sequence) (specifier-type 'simple-bit-vector)))
...
(deftransform count ((item sequence) (t (not bit-vector)) *
                     :important nil)
  (when (and (array-type-p (lvar-type sequence))
             (not (csubtypep (lvar-type item) (array-type-element-type (lvar-type sequence)))))
